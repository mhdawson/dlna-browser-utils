...
const buildRequestXml = function(id, browseFlag, filter, startIndex, requestCount, sort) {
  ...
  return xmlbuilder.create('s:Envelope', { version: '1.0', encoding: 'utf-8' })
        ...
        .ele('ObjectID', id)
        .up().ele('BrowseFlag', browseFlag)
        .up().ele('Filter', filter)
        .up().ele('StartingIndex', startIndex)
        .up().ele('RequestedCount', requestCount)
        .up().ele('SortCriteria', sort)
        .doc().end({ pretty: true, indent: '  '});
}

// function that allow you to browse a DLNA server
var browseServer = function(callback, controlUrl, id, browseFlag, filter, startIndex, requestCount, sort) {
  ...
  req.write(buildRequestXml(id, browseFlag, filter, startIndex, requestCount, sort));
  req.end();
};
